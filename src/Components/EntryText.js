...
class EntryText extends Component {
  render() {
    ...
    return (
      <div className={`align-bottom feedbackf md:rounded-md text-left overflow-hidden shadow-xl transform transition-all sm:align-middle transition shadow-md hover:shadow-2xl focus:shadow-2xl mb-4 md:mb-8 ${hidden ? "hidden" : ""}`}>
              <div className="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="flex items-center">
           
                <div className=" mt-0 ml-4 text-left">
                    <div as="h3" className="text-lg leading-6 font-medium text-red-500">
                    	{title || "Entry Text"}
                    </div>
                    ...
                    {children ? children : <div className="relative">
                      <div className={`text-gray-400 text-xs absolute top-0 right-1`}>
                        ...
                        <textarea 
                          disabled={disabled}
                          maxLength={600}
                          className={`mt-6 focus:outline-none text-lg  feedbkiback rounded-md px-4 py-4  min-w-full border border-gray-300 font-regular h-48 ${disabled ? "bg-gray-200 text-gray-500" : ""}`} 
                          onChange={onChange} 
                          value={prompt} 
