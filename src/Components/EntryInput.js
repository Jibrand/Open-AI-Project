	...
	render() {
		...
		return (
			<div className={`${this.props.size ? "mt-4" : "mt-4"}`}>
				...
				<label htmlFor={this.props.prompt.attr} className="relative transition text-gray-700 focus-within:text-gray-800 block">
				{Icon ? <Icon className={`
				...
				{this.props.prompt.options ? null : <div className={`text-xs absolute z-30 right-2   select-none pointer-events-none transition-all top px-4 ${this.isMinLength ? "text-red-500" : "text-gray-700"} ${(!this.isLongEnoughToDrop && this.props.size) ? "" : ""}`}>
